...
func (r Flux) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Basedir *Directory
	}
	concrete.Basedir = r.Basedir
	return json.Marshal(&concrete)
}

func (r *Flux) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Basedir *Directory
	}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Flux":
		...
		case "Validate":
			...
			return (*Flux).Validate(&parent, ctx)
		case "":
			var parent Flux
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			if inputArgs["basedir"] != nil {
				err = json.Unmarshal([]byte(inputArgs["basedir"]), &basedir)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg basedir", err))
				}
			}
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Flux").
					WithFunction(
						dag.Function("Validate",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Walk through a given directory and check that the manifests are valid")).
					WithField("Basedir", dag.TypeDef().WithObject("Directory"), TypeDefWithFieldOpts{Description: "Base directory to walk through in order to validate Kubernetes manifests."}).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Flux")).
							WithArg("basedir", dag.TypeDef().WithObject("Directory").WithOptional(true), FunctionWithArgOpts{Description: "Base directory to walk through in order to validate Kubernetes manifests."}))), nil
	default:
